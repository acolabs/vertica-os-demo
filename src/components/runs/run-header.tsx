...
export function RunHeader({ run, agentName }: RunHeaderProps) {
  ...
  const stats = [
    ...
    { label: "Tokens Used", value: (run.tokens_used || 0).toLocaleString(), icon: Hash, color: "text-[var(--primary)]" },
    { label: "Cost", value: formatCurrency(run.cost_usd || 0), icon: Coins, color: "text-[var(--text-secondary)]" },
    { label: "Decisions", value: String(run.decisions_created || 0), icon: Zap, color: "text-amber-400" },
  ];

  return (
    <div className="space-y-4">
      <Link href={`/agents/${run.agent_id}`} className="inline-flex items-center gap-1.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">Run {shortId}</h1>
